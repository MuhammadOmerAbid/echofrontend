...
export default function SetupPage() {
  ...
  return (
    <div className="min-h-screen bg-ink flex flex-col items-center justify-center px-6 py-12">
      <div className="font-serif italic text-white text-2xl mb-10">
            ...
            <div className={`flex items-center gap-2 ${step > i + 1 ? "opacity-60" : ""}`}>
              <div
                className={`w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-medium transition-colors ${
                  step === i + 1 ? "bg-sage text-white" : step > i + 1 ? "bg-sage/40 text-white" : "bg-[#2a2c28] text-stone"
                }`}
              >
                {step > i + 1 ? "✓" : i + 1}
              </div>
              <span className={`text-[11px] font-medium ${step === i + 1 ? "text-white" : "text-stone"}`}>{s}</span>
            </div>
            {i < STEPS.length - 1 && (
              <div className={`w-8 h-px ${step > i + 1 ? "bg-sage/40" : "bg-[#2a2c28]"}`} />
            )}
          </div>
        ))}
      </div>
        ...
        <AnimatePresence mode="wait">
          {step === 1 && (
            <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.35 }} className="space-y-5">
              <div>
                <h2 className="font-display font-bold text-white text-2xl mb-1">Name your campus.</h2>
          ...
          {step === 2 && (
            <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.35 }} className="space-y-4">
              <div>
                <h2 className="font-display font-bold text-white text-2xl mb-1">Campus locations.</h2>
                ...
                {locs.map((l, i) => (
                  <div key={i} className="flex gap-2 items-center">
                    <input value={l.name} onChange={(e) => updateLoc(i, "name", e.target.value)} placeholder="Location name"
                      className="flex-1 bg-[#161816] border border-[#2a2c28] text-white text-sm px-3 py-2 rounded-lg focus:outline-none focus:border-sage" />
                    <input value={l.building} onChange={(e) => updateLoc(i, "building", e.target.value)} placeholder="Building"
                      className="flex-1 bg-[#161816] border border-[#2a2c28] text-white text-sm px-3 py-2 rounded-lg focus:outline-none focus:border-sage" />
                    <button onClick={() => removeLoc(i)} className="text-stone hover:text-red-400 text-sm px-2 transition-colors">✕</button>
                  </div>
                ))}
              </div>
              <button onClick={addLoc} className="text-sage2 text-sm hover:text-white transition-colors">+ Add location</button>
          ...
          {step === 3 && (
            <motion.div key="s3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.35 }} className="space-y-5">
              <div>
                <h2 className="font-display font-bold text-white text-2xl mb-1">Set admin password.</h2>
              ...
              <div>
                ...
                {password && (
                  <div className="mt-2 flex gap-1">
                    {[1, 2, 3, 4].map((l) => (
                      <div key={l} className={`h-1 flex-1 rounded-full transition-colors ${password.length >= l * 3 ? "bg-sage" : "bg-[#2a2c28]"}`} />
                    ))}
                  </div>
                )}
              </div>
