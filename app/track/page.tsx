...
const SENT_CONFIG = {
  ...
  pending:  { label: "Processing…", dot: "#8c897f", bg: "#f7f5ef", text: "#8c897f" },
};
      ...
      <div className="flex-1 flex items-start justify-center px-6 pt-20 pb-16">
        <div className="w-full max-w-lg">
          ...
          >
            ...
            <h1 className="font-display font-bold text-ink text-3xl leading-snug mb-3">
              Where is your <span className="text-sage">suggestion?</span>
            </h1>
            ...
            <form onSubmit={handleTrack} className="space-y-3 mb-6">
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full bg-white border border-black/10 focus:border-sage text-ink text-sm px-4 py-3 rounded-xl outline-none transition-colors font-mono placeholder:text-stone/60"
                />
              </div>
              {error && (
            ...
            <AnimatePresence>
              {result && sentCfg && (
                ...
                >
                  ...
                  <div className="bg-white border border-black/[0.08] rounded-2xl p-5">
                    <div className="flex items-center gap-3 mb-4">
                      <div className="w-8 h-8 rounded-full flex items-center justify-center"
                        style={{ background: result.processed ? "#fff1e6" : "rgba(0,0,0,0.06)" }}>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        ...
                        <div key={label} className="bg-paper rounded-xl p-3">
                          <div className="text-[9px] uppercase tracking-[0.2em] text-stone font-semibold mb-1">{label}</div>
                  ...
                  {result.processed && (
                    <div className="bg-white border border-black/[0.08] rounded-2xl p-5">
                      <div className="text-[9px] uppercase tracking-[0.2em] text-stone font-semibold mb-3">
                        Sentiment Analysis
                      </div>
                      <div className="flex items-center gap-3 mb-3">
                        <div className="w-2.5 h-2.5 rounded-full" style={{ background: sentCfg.dot }} />
                        <span className="font-display font-bold text-ink text-xl">{sentCfg.label}</span>
                        <span className="font-mono text-sm text-stone">
                          {result.sentiment_score > 0 ? "+" : ""}{result.sentiment_score.toFixed(3)}
                        </span>
                      </div>
                      <div className="h-2 rounded-full overflow-hidden bg-black/[0.08]">
                      ...
                      <div className="flex justify-between mt-1">
                        <span className="text-[10px] text-stone/60">−1 Negative</span>
                        <span className="text-[10px] text-stone/60">+1 Positive</span>
                      </div>
                    </div>
                  )}
                  ...
                  {result.processed && result.cluster_id >= 0 && (
                    <div className="bg-white border border-black/[0.08] rounded-2xl p-5">
                      ...
                      <div className="flex items-baseline gap-3 mb-2">
                        <span className="font-display font-bold text-ink text-2xl">Cluster {result.cluster_id}</span>
                      </div>
                  ...
                  {result.processed && (
                    <div className="bg-white border border-black/[0.08] rounded-2xl p-5">
                      ...
                      <div className="flex items-baseline gap-2 mb-2">
                        <span className="font-display font-bold text-ink text-2xl">{result.priority_score.toFixed(4)}</span>
                        <span className="text-stone text-xs">/ 1.0000</span>
                      </div>
                      <div className="h-1.5 rounded-full overflow-hidden mb-2 bg-black/[0.08]">
                  ...
                  <div className="flex gap-3 pt-2">
                    <button
                      ...
                      className="border border-black/10 hover:border-black/25 text-stone hover:text-ink rounded-full px-5 py-2.5 text-sm font-medium transition-colors"
                    >
                      Track Another
                    </button>
          ...
          {!result && (
            <div className="mt-10 pt-8 border-t border-black/[0.08]">
              <p className="text-stone/60 text-[10px] uppercase tracking-[0.22em] font-semibold mb-4">How tracking works</p>
  ...
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-paper flex items-center justify-center">
        <p className="font-sans font-light text-sm text-stone">Loading…</p>
      </div>
    }>
      <TrackInner />
    </Suspense>
